...
import { getDuplicate, getNowFlow, getNowTeam, setMatchStageNext, toUnique } from '../match/functions';
...
const Select: CommandFunction<Options_Select> = async (interaction, { operators }) => {
    ...
    if (nowFlow.option == 'ban' || nowFlow.option == 'pick')
        return await banPickRole(interaction, operatorList, match, nowFlow as Flow<BPOption>);
};
